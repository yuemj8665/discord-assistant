...
class InfraScheduler(BaseScheduler):
    """홈서버 리소스 모니터링 + 아침 일일 리포트 (LLM 분석 포함)."""
    ...
    def start(self) -> None:
        self._spawn_loop(self._check_resources, config.INFRA_CHECK_INTERVAL)
    ...
    async def _daily_tick(self) -> None:
        if self._should_run_daily(
            "infra.daily_report",
            config.INFRA_DAILY_REPORT_HOUR,
            config.INFRA_DAILY_REPORT_MINUTE,
        ):
    ...
    async def _send(self, message: str) -> None:
        await self._send_to(config.INFRA_CHANNEL_ID, message)
